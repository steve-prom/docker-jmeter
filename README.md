...
- **JMETER_VERSION** - JMeter version, default ``5.4.1``. Use as env variable to build with another version: `export JMETER_VERSION=5.4`
- **IMAGE_TIMEZONE** - timezone of Docker image, default ``"America/Los_Angeles"``. Use as env variable to build with another timezone: `export IMAGE_TIMEZONE="America/New_York"`
...
1. In a Terminal/Command session, install Git, navigate/make a folder, then:
   ...
   ```
   git clone https://github.com/steve-prom/docker-jmeter.git
   cd docker-jmeter
   ```
...
1. Run the Build script to download dependencies, including the docker CLI:
   ...
   <tt>Successfully tagged promenade/jmeter:5.3</tt>
